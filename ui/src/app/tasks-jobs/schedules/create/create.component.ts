...
export class CreateComponent implements OnInit {
    ...
    buildForm({ tasks, schedules, platforms }) {
        ...
        const names = new FormArray(tasks.map(() => new FormControl('', [
            Validators.required, ((control: FormControl) => SchedulesCreateValidator.existName(control, schedules))
                ...
                .subscribe(() => {
                    if (scheduleParams.length === 1) {
                        this.notificationService
                            .success('Schedule creation', `Successfully schedule creation "${scheduleParams[0].schedulerName}"`);
                    } else {
