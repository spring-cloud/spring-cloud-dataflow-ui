...
export function layout(paper: dia.Paper) {
    ...
    // ignore embedded cells
        if (!node.get('parent')) {
            ...
            if (node.prop('metadata/name') === START_NODE_TYPE && node.prop('metadata/group') === CONTROL_GROUP_TYPE) {
                start = node;
            } else if (node.prop('metadata/name') === END_NODE_TYPE && node.prop('metadata/group') === CONTROL_GROUP_TYPE) {
                end = node;
            } else {
                empty = false;
            }
...
export function verticalLinksOverlapping(g: dagre.graphlib.Graph, tolerance: number, ranksOffset?: any): number {
    ...
    for (let i = 0; i < verticalLinks.length - 1; i++) {
        ...
        for (let j = i + 1; j < verticalLinks.length; j++) {
            ...
            if ((minX < otherSource.x + offsetSourceC && otherSource.x + offsetSourceC < maxX && minY < otherSource.y
                && otherSource.y < maxY) || (minX < otherTarget.x + offsetTargetC && otherTarget.x + offsetTargetC < maxX
                    && minY < otherTarget.y && otherTarget.y < maxY)) {
                numberOfOverlaps++;
            }
        }
    }
...
export function centerAlignRanks(g: dagre.graphlib.Graph): any {
    ...
    Object.keys(ranks).forEach(key => {
        ...
        if (Array.isArray(rank) && rank.length) {
            let rankMinX = Number.POSITIVE_INFINITY;
            let rankMaxX = Number.NEGATIVE_INFINITY;
            rank.forEach(n => {
