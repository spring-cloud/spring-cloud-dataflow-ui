...
const METAMODEL_DATA: Array<RawMetadata> = [
  {
    ...
    options: [{id: 'port', name: 'port', defaultValue: '80', description: 'Port on which to listen', type: 'number'}]
  },
  {
    name: 'time-app',
    type: 'app',
  ...
  },
  {
    name: 'http',
    type: 'source',
    description: 'Receive HTTP input',
    options: [{id: 'port', name: 'port', defaultValue: '80', description: 'Port on which to listen', type: 'number'}]
  },
  {
    name: 'rabbit',
    ...
    options: [
      {id: 'queue', name: 'queue', description: 'the queue(s) from which messages will be received'},
      {
        id: 'time-unit',
        name: 'time-unit',
        description: 'Time unit for heart beat messages',
        type: 'enum',
        options: ['HOURS', 'MINUTES', 'SECONDS', 'MILIOSECONDS'],
  ...
  },
  {
    name: 'filewatch',
    type: 'source',
    description: 'Produce messages from the content of files created in a directory',
    options: [{id: 'dir', name: 'dir', description: 'the absolute path to monitor for files'}]
  },
  {
    name: 'transform',
    type: 'processor',
    description: 'Apply an expression to modify incoming messages',
    options: [{id: 'expression', name: 'expression', defaultValue: 'payload', description: 'SpEL expression to apply'}]
  },
  {
    name: 'filter',
    type: 'processor',
    description: 'Only allow messages through that pass the filter expression',
    options: [
      {
        id: 'expression',
        name: 'expression',
  ...
  },
  {
    name: 'ftp',
    type: 'sink',
    description: 'Send messages over FTP',
    options: [
  ...
  {
    name: 'super-very-very-very-looooooooong-task-name',
    type: 'task',
    description: 'Task d',
    options: []
  },
...
export class MockSharedAppService extends AppService {
  ...
  getApps(
    ...
    const apps = METAMODEL_DATA.filter(d => !type || d.type === type.toString()).map(App.parse);
    appPage.items = apps;
    appPage.page = 0;
