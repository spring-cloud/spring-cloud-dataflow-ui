...
export class GrafanaServiceMock {
  ...
  getDashboardStream(stream: Stream): Observable<string> {
    return of(
      'http://localhost:3000/d/scdf-applications/applications?refresh=15s&var-stream_name=' +
        stream.name +
  ...
  getDashboardTask(task: Task): Observable<string> {
    return of('http://localhost:3000/d/scdf-tasks/tasks?refresh=15s&var-task_name=' + task.name + '&var-task_name=All');
  }

  getDashboardTaskExecution(execution: TaskExecution): Observable<string> {
    return of(
      'http://localhost:3000/d/scdf-tasks/tasks?refresh=15s&var-task_name=' +
        execution.taskName +
        '&var-task_name=All' +
  ...
  getDashboardJobExecution(jobExecution: JobExecution): Observable<string> {
    return of(
      'http://localhost:3000/d/scdf-tasks/tasks?refresh=15s&var-job_name=' +
        jobExecution.name +
