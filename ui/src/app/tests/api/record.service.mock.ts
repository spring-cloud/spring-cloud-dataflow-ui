...
export class RecordServiceMock {
  ...
  getOperationTypes(): Observable<RecordOperationType[]> {
    return of(GET_OPERATION_TYPES).pipe(
      delay(1),
      map(items => items.map(RecordOperationType.parse))
    );
  }

  getActionTypes(): Observable<RecordActionType[]> {
    return of(GET_ACTION_TYPES).pipe(
      delay(1),
      map(items => items.map(RecordActionType.parse))
    );
  }
