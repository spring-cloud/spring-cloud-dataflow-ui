...
export class BuilderComponent implements OnInit, OnDestroy {
                ...
                if ((appKey !== '*' && appNames.indexOf(appKey) === -1) || keyReduce === '') {
                    ...
                    if (appNames.indexOf(appKey) > -1) {
                        ...
                        if (appProperties.options && !appProperties.optionsState.isInvalid) {
                            const option = builder.builderAppsProperties[appKey]
                                .find(opt => opt.name === keyReduce || opt.id === keyReduce);
                            if (option) {
                                option.value = value;
                                free = false;
                            }
   ...
   */
    openDeploymentProperties(builder, appId?: string) {
        ...
        groupedPropertiesSources = groupedPropertiesSources.sort(((a, b) => a.title === b.title ? 0 : a.title < b.title ? -1 : 1));
