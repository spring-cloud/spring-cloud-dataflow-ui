...
export class TaskService {
  ...
  createTask(name: string, definition: string, description: string): Observable<any> {
    ...
    const headers = HttpUtils.getDefaultHttpHeaders();
    return this.httpClient.post(UrlUtilities.calculateBaseApiUrl() + 'tasks/definitions', {}, {headers, params}).pipe(catchError(ErrorUtils.catchError));
  }
  ...
  launch(taskName: string, args: string, props: string): Observable<number | unknown> {
    ...
    return this.httpClient.post<string>(UrlUtilities.calculateBaseApiUrl() + 'tasks/executions', {}, {headers, params}).pipe(
      map(body => {
  ...
  taskExecutionsClean(task: Task, completed: boolean): Observable<any> {
    ...
    const url = UrlUtilities.calculateBaseApiUrl() + `tasks/executions?action=CLEANUP,REMOVE_DATA${paramCompleted}${paramTask}`;
    return this.httpClient.delete<any>(url, {headers, observe: 'response'}).pipe(catchError(ErrorUtils.catchError));
  }
  ...
  getExecutionLogs(taskExecution: TaskExecution): Observable<any> {
    ...
    return this.httpClient
      .get<any>(UrlUtilities.calculateBaseApiUrl() + `tasks/logs/${taskExecution.externalExecutionId}${platform}`, {headers})
      .pipe(catchError(ErrorUtils.catchError));
  }
