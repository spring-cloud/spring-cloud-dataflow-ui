...
export class StreamService {
  ...
  getStreamsRelated(streamName: string, nested?: boolean): Observable<Stream[] | unknown> {
    ...
    return this.httpClient.get<any>(UrlUtilities.calculateBaseApiUrl() + `streams/definitions/${streamName}/related`, {params, headers}).pipe(
  ...
  createStream(name: string, dsl: string, description: string): Observable<any> {
    ...
    return this.httpClient
      .post<any>(UrlUtilities.calculateBaseApiUrl() + 'streams/definitions', null, {headers, observe: 'response', params})
      .pipe(catchError(ErrorUtils.catchError));
  }
  ...
  deployStream(name: string, propertiesAsMap: any = {}): Observable<HttpResponse<any> | unknown> {
    const headers = HttpUtils.getDefaultHttpHeaders();
    return this.httpClient
      .post<any>(UrlUtilities.calculateBaseApiUrl() + `streams/deployments/${name}`, propertiesAsMap, {headers, observe: 'response'})
      .pipe(catchError(ErrorUtils.catchError));
  }
  ...
  getLogs(name: string): Observable<any> {
    const headers = HttpUtils.getDefaultHttpHeaders();
    return this.httpClient.get<any>(UrlUtilities.calculateBaseApiUrl() + `streams/logs/${name}`, {headers}).pipe(catchError(ErrorUtils.catchError));
  }
  ...
  getStreamHistory(name: string): Observable<StreamHistory[]> {
    const headers = HttpUtils.getDefaultHttpHeaders();
    return this.httpClient.get<any>(UrlUtilities.calculateBaseApiUrl() + `streams/deployments/history/${name}`, {headers}).pipe(
  ...
  rollbackStream(streamHistory: StreamHistory): Observable<HttpResponse<any>> {
    const headers = HttpUtils.getDefaultHttpHeaders();
    return this.httpClient
      .post<any>(UrlUtilities.calculateBaseApiUrl() + `streams/deployments/rollback/${streamHistory.stream}/${streamHistory.version}`, {headers})
      .pipe(catchError(ErrorUtils.catchError));
  }
}
