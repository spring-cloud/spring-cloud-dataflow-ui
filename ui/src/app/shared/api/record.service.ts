...
export class RecordService {
  ...
  getOperationTypes(): Observable<RecordOperationType[] | unknown> {
    if (this.operationTypes) {
      return of(this.operationTypes);
    }
    return this.httpClient
      .get<any>(UrlUtilities.calculateBaseApiUrl() + 'audit-records/audit-operation-types', {
  ...
  getActionTypes(): Observable<RecordActionType[] | unknown> {
    if (this.actionTypes) {
      return of(this.actionTypes);
    }
    return this.httpClient
      .get<any>(UrlUtilities.calculateBaseApiUrl() + 'audit-records/audit-action-types', {
        headers: HttpUtils.getDefaultHttpHeaders()
      })
      .pipe(
        map(response => {
