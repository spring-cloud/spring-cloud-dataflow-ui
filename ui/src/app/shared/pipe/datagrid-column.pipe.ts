...
export class DatagridColumnPipe implements PipeTransform {
  ...
  async transform(value: any, datagrid: ClrDatagrid, contextName: string, gap = 10): Promise<any> {
    if (value) {
      return +value;
    }
    const context = await this.contextService.getContext(contextName).pipe(take(1)).toPromise();
    const cols = [];
    ...
    const width =
      datagrid?.datagridTable?.nativeElement?.clientWidth - cols.reduce((prev, current) => prev + current, 0);
