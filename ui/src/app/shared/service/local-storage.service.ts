...
export class LocalStorageService {
  ...
  get<T>(key: string): T | null {
    const item = this.localStorage?.getItem(`dataflow-${key}`);
  ...
  set(key: string, value: unknown): void {
    this.localStorage?.setItem(`dataflow-${key}`, JSON.stringify(value));
  }
  ...
  removeKeys(keys: string[]): void {
    keys.forEach(key => this.localStorage?.removeItem(`dataflow-${key}`));
  }
