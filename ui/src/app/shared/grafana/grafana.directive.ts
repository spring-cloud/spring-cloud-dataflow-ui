...
export class GrafanaStreamsDirective implements OnInit {
  ...
  ngOnInit(): void {
    this.grafanaService.isAllowed().subscribe(allow => {
      this.hidden = !allow;
    });
  }
...
export class GrafanaStreamDirective implements OnInit {
  ...
  @HostListener('click') onClick(): void {
    this.grafanaService.getDashboardStream(this.stream).subscribe((url: string) => {
      window.open(url);
    });
  }
}
...
export class GrafanaTaskDirective implements OnInit {
  ...
  @HostListener('click') onClick(): void {
    this.grafanaService.getDashboardTask(this.task).subscribe((url: string) => {
      window.open(url);
    });
  }
}
...
export class GrafanaRuntimeAppDirective implements OnInit {
  ...
  @HostListener('click') onClick(): void {
    this.grafanaService.getDashboardApplication(this.streamName, this.appName).subscribe((url: string) => {
      window.open(url);
    });
  }
}
...
export class GrafanaTaskExecutionDirective implements OnInit {
  ...
  @HostListener('click') onClick(): void {
    this.grafanaService.getDashboardTaskExecution(this.taskExecution).subscribe((url: string) => {
      window.open(url);
    });
  }
}
...
export class GrafanaJobExecutionDirective implements OnInit {
  ...
  @HostListener('click') onClick(): void {
    this.grafanaService.getDashboardJobExecution(this.jobExecution).subscribe((url: string) => {
      window.open(url);
    });
  }
}
