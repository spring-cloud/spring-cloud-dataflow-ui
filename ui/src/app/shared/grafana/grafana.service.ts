...
export class GrafanaService {
    ...
    getDashboardStreams(): Observable<string> {
        return this.aboutService
            .getMonitoring()
            .pipe(
                map((info: any): string => `${info.url}/d/scdf-streams/streams?refresh=${info.refreshInterval}s`)
            );
    }
    ...
    getDashboardStream(stream: Stream): Observable<string> {
        return this.aboutService
            .getMonitoring()
            .pipe(
                map((info: any): string => info.url + '/d/scdf-applications/applications?refresh=' + info.refreshInterval +
    ...
    getDashboardTasks(): Observable<string> {
        return this.aboutService
            .getMonitoring()
            .pipe(
                map((info: any): string => info.url + '/d/scdf-tasks/tasks?refresh=' + info.refreshInterval + 's')
            );
    }
    ...
    getDashboardTask(task: Task): Observable<string> {
        return this.aboutService
            .getMonitoring()
            .pipe(
                map((info: any): string => info.url + '/d/scdf-tasks/tasks?refresh=' + info.refreshInterval +
